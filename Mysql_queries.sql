...
select title, count(inventory_id) as copy_count
...
select first_name, last_name, sum(amount) as paid_total
...
select store_id, sum(amount) as rental_income
from store
...
select name as genre, sum(amount) as rental_income
from category
